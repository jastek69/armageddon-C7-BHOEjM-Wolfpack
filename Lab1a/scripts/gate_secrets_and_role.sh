...
usage() {
  cat <<EOF
Usage:
  REGION=us-east-2 INSTANCE_ID=i-077f56b5bcfe86a1c SECRET_ID=lab1asecret1 ./gate_secrets_and_role.sh
...
Examples:
  ...
  REQUIRE_ROTATION=true REGION=us-east-2 INSTANCE_ID=i-123 SECRET_ID=lab1asecret1 ./gate_secrets_and_role.sh

  # Run on the EC2 and verify it can read the secret value (does NOT print it):
  CHECK_SECRET_VALUE_READ=true REGION=us-east-2 INSTANCE_ID=i-123 SECRET_ID=lab1asecret1 ./gate_secrets_and_role.sh
EOF
}
