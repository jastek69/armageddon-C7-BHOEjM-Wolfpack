...
resource "aws_route53_record" "zone_apex_alias01" {
...
#already had an  A record to 1c-bonus_b.tf before I saw this.
...
resource "aws_s3_bucket" "maximus_alb_logs_bucket01" {
  count = var.enable_alb_access_logs ? 1 : 0

  bucket = "${local.project_name}-alb-logs-${data.aws_caller_identity.current_self01.account_id}"

  force_destroy = true
  tags = {
    Name = "${local.project_name}-alb-logs-bucket01"
  }
}

# Explanation: Block public access—Chewbacca does not publish the ship’s black box to the galaxy.
resource "aws_s3_bucket_public_access_block" "maximus_alb_logs_pab01" {
  count = var.enable_alb_access_logs ? 1 : 0

  bucket                  = aws_s3_bucket.maximus_alb_logs_bucket01[0].id #I don't think we need the [0] here since we are only creating one bucket, but it is needed to access the bucket when count is used.
...
resource "aws_s3_bucket_ownership_controls" "maximus_alb_logs_owner01" {
  count = var.enable_alb_access_logs ? 1 : 0

  bucket = aws_s3_bucket.maximus_alb_logs_bucket01[0].id
  rule {
...
resource "aws_s3_bucket_policy" "maximus_alb_logs_policy01" {
  count = var.enable_alb_access_logs ? 1 : 0
    ...
    Statement = [
      {
        ...
        Action    = "s3:*"
        Resource = [
          aws_s3_bucket.maximus_alb_logs_bucket01[0].arn,
          "${aws_s3_bucket.maximus_alb_logs_bucket01[0].arn}/*"
        ]
        Condition = {
      ...
      },
      {
        ...
        Action   = "s3:PutObject"
        Resource = "${aws_s3_bucket.maximus_alb_logs_bucket01[0].arn}/${var.alb_access_logs_prefix}/AWSLogs/${data.aws_caller_identity.current_self01.account_id}/*"
      }
    ]
  })
}
