...
CF_DISTRIBUTION_ID="E2I43XW8O3TESS"

# Domain you are serving (e.g. chewbacca-growls.com)
DOMAIN_NAME="aster-project.site"

# Route53 Hosted Zone ID for your domain (e.g. Z123ABC...)
ROUTE53_ZONE_ID="Z025267937HGJZCJYH3TC"
...
if [[ "$REQUIRE_WAF_ASSOCIATION" == "true" ]]; then
  ...
  if [[ -n "$waf_assoc_arn" && "$waf_assoc_arn" != "None" ]]; then
    ...
    managed_count="$(aws wafv2 get-web-acl \
      --id "$(echo "$waf_assoc_arn" | awk -F/ '{print $NF}')" \
      --name "$(echo "$waf_assoc_arn" | awk -F/ '{print $(NF-1)}')" \
      --scope CLOUDFRONT \
      --region "$CF_ACM_REGION" \
      --query "length(WebACL.Rules[?Statement.ManagedRuleGroupStatement!=null])" \
...
check_alias_record() {
 ...
 # Normalize values to avoid CRLF / trailing-dot weirdness
clean_target="$(echo "$target" | tr -d '\r' | sed 's/\.$//')"
clean_found_targets="$(echo "$found_target" | tr '\t' '\n' | tr -d '\r' | sed 's/\.$//')"

printf 'DEBUG type=<%s>\n' "$type"
printf 'DEBUG target=<%q>\n' "$target"
printf 'DEBUG found_target=<%q>\n' "$found_target"
printf 'DEBUG found_zone=<%q>\n' "$found_zone"

if echo "$clean_found_targets" | grep -qix "$clean_target"; then
  add_detail "PASS: Route53 $type alias points to CloudFront ($clean_target)."
else
  add_failure "FAIL: Route53 $type alias does not point to CloudFront (expected=$clean_target, actual=$found_target)."
fi
