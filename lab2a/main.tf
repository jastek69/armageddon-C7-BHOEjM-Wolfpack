...
# Explanation: lab1c only opens the hangar door — allow ALB -> EC2 on app port (e.g., 80).
resource "aws_security_group_rule" "lab1c_ec2_ingress_from_alb01" {
  type                     = "ingress"
  security_group_id        = aws_security_group.lab1c_ec2_sg01.id
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.lab1c_alb_sg01.id
