...
locals {
  ...
  #use data in 1c-bonus.tf "Error: no matching Route 53 Hosted Zone found" works with dns lab
  dns_zone_id = var.manage_route53_in_terraform ? aws_route53_zone.dns_zone01[0].zone_id : var.route53_hosted_zone_id

  # Explanation: This is the app address that will growl at the galaxy (app.chewbacca-growl.com).
  dns_app_fqdn = "${var.app_subdomain}.${var.domain_name}"
}
...
resource "aws_route53_zone" "dns_zone01" {
  count = var.manage_route53_in_terraform ? 1 : 0
  #variable not included/ used data
  name = local.dns_zone_name

  tags = {
    Name = "${local.project_name}-zone01"
  }
}
...
resource "aws_route53_record" "dns_acm_validation_records01" {
  for_each = var.certificate_validation_method == "DNS" ? {
    for dvo in aws_acm_certificate.domain_acm_cert01.domain_validation_options :
    dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "dns_acm_validation01_dns_bonus" {
  count = var.certificate_validation_method == "DNS" ? 1 : 0

  certificate_arn = aws_acm_certificate.domain_acm_cert01.arn

  validation_record_fqdns = [
    for r in aws_route53_record.dns_acm_validation_records01 : r.fqdn
  ]
}
