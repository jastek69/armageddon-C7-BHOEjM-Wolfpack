...
output "lab1c_apex_url_https" { value = "https://${var.domain_name}" }

output "lab1c_alb_logs_bucket_name" { 
  value = var.enable_alb_access_logs ? aws_s3_bucket.lab1c_alb_logs_bucket01[0].bucket : null 
  }

output "lab1c_waf_log_destination" { value = var.waf_log_destination }



output "lab1c_waf_logs_s3_bucket" { 
  value = var.waf_log_destination == "s3" ? aws_s3_bucket.aws-waf-logs-lab1c-bucket01[0].bucket : null 
  }