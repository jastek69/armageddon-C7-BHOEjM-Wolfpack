...
SECRET_ID = os.environ.get("SECRET_ID", "lab1c/secret10")
...
def get_conn():
    ...
    db = c.get("dbname", "lab1cdb")  # we'll create this if it doesn't exist
...
def init_db():
    ...
    cur = conn.cursor()
    cur.execute("CREATE DATABASE IF NOT EXISTS lab1cdb;")
    cur.execute("USE lab1cdb;")
    cur.execute("""
    ...
    cur.close()
    conn.close()
    return "Initialized lab1cdb + notes table."
...
Environment=SECRET_ID=lab1c/secret10
