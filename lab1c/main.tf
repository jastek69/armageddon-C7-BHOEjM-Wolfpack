...
resource "aws_vpc_security_group_ingress_rule" "ec2-sg-ssh" {
...
resource "aws_vpc_security_group_ingress_rule" "http-from-ec2" {
  ...
  referenced_security_group_id = "sg-0563a5b83b2a393fa"
  from_port         = 3306
  ip_protocol       = "tcp"
  to_port           = 3306
...
resource "aws_iam_role_policy" "lab1c_secret_read_inline01" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = aws_secretsmanager_secret.lab1c_db_secret10.arn
    }]
  })
}
...
resource "aws_instance" "lab1c_ec201" {
  ami                    = var.ec2_ami_id
  instance_type           = var.ec2_instance_type
  key_name               = aws_key_pair.lab1c_keypair.key_name
...
resource "aws_secretsmanager_secret" "lab1c_db_secret10" {
  name = "${local.name_prefix}/secret10"
}
...
resource "aws_secretsmanager_secret_version" "lab1c_db_secret_version01" {
  secret_id = aws_secretsmanager_secret.lab1c_db_secret10.id
