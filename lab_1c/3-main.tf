...
resource "aws_iam_role_policy_attachment" "lab1c_ec2_logs_read_attach" {
...
resource "aws_secretsmanager_secret" "lab1c_db_secret" {
  name = "${local.name_prefix}/rds/mysql_v15"
}
...
resource "aws_cloudwatch_log_metric_filter" "lab1c_db_error_filter" {
  name           = "${local.name_prefix}-db-error-filter"
  log_group_name = aws_cloudwatch_log_group.lab1c_log_group.name

  # Match any log line containing ERROR
  pattern = "ERROR"
...
#####test###
resource "aws_cloudwatch_metric_alarm" "lab1c_db_log_alarm" {
  alarm_name          = "${local.name_prefix}-db-log-errors"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  metric_name         = "DBLogErrors"
  namespace           = "Lab/RDSApp"
  period              = 60
  statistic           = "Sum"
  threshold           = 1
  ...
  tags = {
    Name = "${local.name_prefix}-alarm-db-log-errors"
  }
}
####/test###
...
resource "aws_sns_topic" "lab1c_sns_topic" {
...
resource "aws_sns_topic_subscription" "lab1c_sns_sub" {
  topic_arn = aws_sns_topic.lab1c_sns_topic.arn
  protocol  = "email"
