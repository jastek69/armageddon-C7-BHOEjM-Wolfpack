...
locals {
  ...
  lab1c_fqdn = "${var.app_subdomain}.${var.domain_name}"
}
...
resource "aws_security_group" "lab1c_alb_sg01" {
...
resource "aws_vpc_security_group_ingress_rule" "http" {
  ...
  security_group_id = aws_security_group.lab1c_alb_sg01.id
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 80
  ip_protocol       = "tcp"
  to_port           = 80
...
resource "aws_vpc_security_group_egress_rule" "alb_to_targets_http" {
  description                  = "ALB private targets on HTTP"
  security_group_id            = aws_security_group.lab1c_alb_sg01.id
  cidr_ipv4                    = "0.0.0.0/0"
  ip_protocol                  = "tcp"
  from_port                    = 80
...
resource "aws_lb" "lab1c_alb01" {
  name               = "${var.project_name}-alb01"
  load_balancer_type = "application"
  internal           = false

  security_groups = [aws_security_group.lab1c_alb_sg01.id]
  subnets         = aws_subnet.lab1c_public_subnets[*].id
...
# Explanation: Target groups are lab1c’s “who do I forward to?” list — private EC2 lives here.
resource "aws_lb_target_group" "lab1c_tg01" {
...
# Explanation: lab1c personally introduces the ALB to the private EC2 — “this is my friend, don’t shoot.”
resource "aws_lb_target_group_attachment" "lab1c_tg_attach01" {
  target_group_arn = aws_lb_target_group.lab1c_tg01.arn
  target_id        = aws_instance.lab1c_ec2.id
  port             = 80
...
# resource "aws_route53_record" "lab1c_acm_validation" { ... }

# Explanation: Once validated, ACM becomes the “green checkmark” — until then, ALB HTTPS won’t work.
resource "aws_acm_certificate_validation" "lab1c_acm_validation01" {
  certificate_arn = aws_acm_certificate.lab1c_acm_cert01.arn

  # TODO: if using DNS validation, students must pass validation_record_fqdns
  # validation_record_fqdns = [aws_route53_record.lab1c_acm_validation.fqdn]
}
...
resource "aws_lb_listener" "lab1c_http_listener01" {
  load_balancer_arn = aws_lb.lab1c_alb01.arn
  port              = 80
  protocol          = "HTTP"
...
resource "aws_lb_listener" "lab1c_https_listener01" {
  load_balancer_arn = aws_lb.lab1c_alb01.arn
  port              = 443
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-TLS13-1-2-2021-06"
  certificate_arn   = aws_acm_certificate_validation.lab1c_acm_validation01.certificate_arn
  ...
  depends_on = [aws_acm_certificate_validation.lab1c_acm_validation01]
}
...
resource "aws_wafv2_web_acl" "lab1c_waf01" {
...
resource "aws_wafv2_web_acl_association" "lab1c_waf_assoc01" {
  count = var.enable_waf ? 1 : 0

  resource_arn = aws_lb.lab1c_alb01.arn
  web_acl_arn  = aws_wafv2_web_acl.lab1c_waf01[0].arn
}
...
resource "aws_cloudwatch_metric_alarm" "lab1c_alb_5xx_alarm01" {
  ...
  dimensions = {
    LoadBalancer = aws_lb.lab1c_alb01.arn_suffix
  }
...
# Explanation: Dashboards are your cockpit HUD — lab1c wants dials, not vibes.
resource "aws_cloudwatch_dashboard" "lab1c_dashboard01" {
    ...
    widgets = [
      {
        ...
        properties = {
          metrics = [
            [ "AWS/ApplicationELB", "RequestCount", "LoadBalancer", aws_lb.lab1c_alb01.arn_suffix ],
            [ ".", "HTTPCode_ELB_5XX_Count", ".", aws_lb.lab1c_alb01.arn_suffix ]
          ]
          period = 300
          stat   = "Sum"
          region = var.aws_region
          title  = "lab1c ALB: Requests + 5XX"
        }
      },
      {
        type  = "metric"
        ...
        properties = {
          metrics = [
            [ "AWS/ApplicationELB", "TargetResponseTime", "LoadBalancer", aws_lb.lab1c_alb01.arn_suffix ]
          ]
          period = 300
          stat   = "Average"
          region = var.aws_region
          title  = "lab1c ALB: Target Response Time"
        }
      }
    ]
  })
